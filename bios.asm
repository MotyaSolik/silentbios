...
.fn_41:
    ...
    ; BX=0xAA55 - настоящий документированный вывод AH=0x41, а
    ; patch_stack_cf безусловно портит BL как часть своего протокола
    ; (0/1 сигнал в BL - "не сохраняется" для всех остальных функций
    ; int13h, но здесь как раз сохранить обязаны) - сохраняем на
    ; стеке и забираем обратно уже после патча CF.
    push bx
    cmp ah, 0x01
    je .ext_ok
    mov bl, 1
...
;
; Использует AX (не BX!) как scratch для чтения/записи байта FLAGS -
; сохраняет и восстанавливает его через push/pop, так что для
; вызывающего это прозрачно. Раньше здесь была BH, что работало,
; пока ни одна функция не возвращала что-то полезное в BX - но
; AH=0x41 (int13h_check_extensions) по спецификации обязана вернуть
; BX=0xAA55, и "прозрачным" scratch-регистром без сохранения он не
; является. Поймано тестом: AH=0x41 честно приходил с CF=0 и
; AH=0x01, но BX неизменно приходил испорченным - оказалось, именно
; этой строкой, а не самим int13h_check_extensions.
patch_stack_cf:
    and bl, 1
    push bp
    push ax
    mov bp, sp
    ; [bp+0]=наш push ax, [bp+2]=наш push bp, [bp+4]=наш возвратный
    ; адрес (call), [bp+6]=IP(оригинал), [bp+8]=CS(оригинал),
    ; [bp+10]=младший байт FLAGS(оригинал, в нём и лежит CF - бит 0)
    mov al, [ss:bp+10]
    and al, 0xFE
    or al, bl
    mov [ss:bp+10], al
...
; Общее ядро LBA-чтения секторов через ATA PIO - переиспользуется
; AH=0x02 (после трансляции CHS->LBA выше) и AH=0x42 (LBA уже в DAP,
; см. int13h_read_sectors_lba). Вход: CX:BX = LBA (биты0-15:16-31),
; BP = число секторов, ES:DI = буфер назначения, RAM_GEOM_DRIVEBIT
; уже выставлен (0=master, 0x10=slave). Выход: AL=число реально
; прочитанных секторов, AH=код статуса, BL=0/1 для patch_stack_cf.
; Портит SI (счётчик), CX/BX (LBA продвигается по ходу чтения),
; DI (буфер продвигается).
ata_lba_transfer:
...
; int 13h AH=0x41 - проверка поддержки расширений (EDD/LBA-доступа).
; Вход: BX=0x55AA (сигнатура запроса), DL=диск. Выход при поддержке:
; CF=0, BX=0xAA55, AH=0x01 ("версия 1.x"), CX=1 (бит0 - "базовый"
; extended disk access subset, AH=0x42-0x44/0x47/0x48 по спеке -
; честно только 0x42 реально реализован, см. комментарий у
; int13h_read_sectors_lba, но более мелкой градации, чем один бит на
; всю группу, спецификация не даёт). Если BX не 0x55AA - честно
; "не поддерживается" (CF=1), как и полагается по спецификации, а не
; молча продолжаем как для правильного запроса.
; Не выставляет BL сама (в отличие от большинства остальных функций
; int13h) - у неё самой BX целиком занят документированным выводом
; (0xAA55), а BL исторически используется как сигнал 0/1 для
; patch_stack_cf. Сигнал для patch_stack_cf вызывающий (int13h_isr
; .fn_41) вычисляет сам, по AH, уже ПОСЛЕ этого вызова.
...
; int 13h AH=0x42 - расширенное чтение через Disk Address Packet
; (DAP). Вход: DL=диск, DS:SI указывает на структуру (стандартный
; 16-байтный вариант):
;   +0x00 (1) размер пакета - не проверяем (см. ниже, почему)
;   +0x01 (1) резерв
;   +0x02 (2) число секторов для чтения
;   +0x04 (2) offset буфера назначения
;   +0x06 (2) segment буфера назначения
;   +0x08 (4) начальный LBA, младшие 32 бита
;   +0x0C (4) начальный LBA, старшие 32 бита - у нас 28-битная
;              адресация (тот же протокол, что и в int13h_read_sectors/
;              ata_read_mbr), т.е. для любого реально адресуемого
;              сектора эти биты и так нулевые; отдельно не проверяем,
;              как и размер пакета выше - оба поля отличают этот
;              вариант DAP от 24-байтного (с плоским 64-битным
;              указателем на буфер, для буферов вне conventional-
;              памяти), которого этот проект не поддерживает и не
;              может отличить иначе, кроме как по этим же полям,
;              так что дополнительная проверка ничего не защитит.
; Выход: AH=код статуса, BL=0/1 для patch_stack_cf - в отличие от
; AH=0x02, спецификация AH=0x42 не возвращает число реально
; прочитанных секторов при частичном успехе, только общий статус.
;
; AH=0x43 (запись) сознательно не реализован - как и у AH=0x03 (CHS-
; запись) в этом проекте, запись на диск вообще нигде не сделана.
; AH=0x41 всё равно заявляет поддержку всей группы (см. её
; комментарий, спецификация не даёт более мелкой градации) - AH=0x43
; при реальной попытке честно провалится через общий "неподдерживаемая
; функция" хвост в int13h_isr, как и было бы для любой другой
; нереализованной функции.
int13h_read_sectors_lba:
    push si
    push di
    push bp
    push es
    push dx                     ; настоящий AH=0x42 не трогает DL на
                                   ; выходе, а ata_lba_transfer (как и
                                   ; ata_read_sectors) многократно
                                   ; переиспользует DX как адрес порта
                                   ; (0x1F0-0x1F7) - без сохранения
                                   ; вызывающий получил бы обратно
                                   ; DL=0xF0 вместо своего номера диска.
                                   ; Тот же баг, что уже ловился и
                                   ; чинился в int13h_read_sectors -
                                   ; здесь его пришлось поймать заново,
                                   ; забыл перенести фикс в новую
                                   ; функцию (см. claude.md).

    mov bp, [si+2]                  ; bp = число секторов
    mov di, [si+4]                   ; di = offset буфера
    mov ax, [si+6]                    ; ax = segment буфера
    mov cx, [si+8]                     ; cx = LBA биты0-15
    mov bx, [si+10]                     ; bx = LBA биты16-31
    mov es, ax                            ; ES:DI готовы как буфер назначения
                                            ; (ax дальше не нужен - его
                                            ; значение уже забрано в es)

    ; --- master/slave по DL - тот же принцип, что и в
    ; int13h_read_sectors (см. её подробный комментарий) ---
    xor al, al
    cmp dl, 0x81
    jne .drivebit_done
    mov al, 0x10
.drivebit_done:
    mov [ss:RAM_GEOM_DRIVEBIT], al
